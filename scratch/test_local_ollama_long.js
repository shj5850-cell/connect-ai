...
async function runTest() {
    ...
    const payload = {
        ...
        stream: false,
        options: { num_ctx: 4096, num_predict: 2048, temperature: 0.8 }
    };

    console.log("Calling Ollama with 180s timeout...");
    try {
        const start = Date.now();
        const response = await axios.post(`${ollamaUrl}/api/chat`, payload, { timeout: 180000 });
