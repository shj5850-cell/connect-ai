// Test script to verify YouTube Trend Intelligence Engine logic (ES Module format)
...
// Manual env loader to bypass dotenv import issue
const envLocalPath = path.join(__dirname, '..', 'viewer-app', '.env.local');
...
// Import ES modules from Next.js viewer-app
import { searchYoutubeMarket, extractTrendDNA, evaluateScriptNovelty } from '../viewer-app/app/lib/trendEngine.js';
...
async function testRun() {
  ...
  const mockDbExists = fs.existsSync(path.join(__dirname, '..', '_company', 'trend_dna_mock_db.json'));
  ...
  if (fs.existsSync(realDbPath)) {
    const content = JSON.parse(fs.readFileSync(realDbPath, 'utf-8'));
