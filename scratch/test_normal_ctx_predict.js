...
async function runTest() {
    ...
    const payload = {
        ...
        stream: false,
        options: { num_ctx: 8192, num_predict: 2048, temperature: 0.8 }
    };

    console.log("Calling Ollama with normal context + num_predict: 2048...");
    try {
        const start = Date.now();
        const response = await axios.post(`${ollamaUrl}/api/chat`, payload, { timeout: 120000 });
        console.log(`Success in ${Date.now() - start}ms`);
        console.log("Response length:", response.data.message.content.length);
        console.log("Response starting:", response.data.message.content.substring(0, 300));
    } catch (e) {
